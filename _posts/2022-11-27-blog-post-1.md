...
$$
E^2-(mc^2)^2-|c\vec{p}|^2=(E+\alpha_0mc^2+c\vec{\alpha}\cdot\vec{p})(E-\alpha_0mc^2-c\vec{\alpha}\cdot\vec{p})=0
$$

which can be done if $\{\alpha_i,\alpha_j\}=2\delta_{ij}$ is anticommutational. So Paili matrices is a natrual choice. Then we have _Dirac equation_: 

$$
i\hbar\frac{\partial}{\partial t}|\psi\rangle=H|\psi\rangle=(\alpha_0mc^2+c\vec{\alpha}\cdot\vec{p})|\psi\rangle
$$
...
Aware the conservative energy 

$$
E^2=(mc^2)^2-|c\vec{\alpha}\cdot\vec{p}|^2.
$$

**Magnetic Effect** To account for magnetic field, we can have a vector potential, and then: $E^2=(mc^2)^2+\vert c\vec{\alpha}\cdot(\vec{p}-q\vec{A})\vert^2$. Assume $\vert c\vec{\alpha}\cdot(\vec{p}-q\vec{A})\vert$ is comparably smaller than the first, we have $E=\sqrt{(mc^2)^2+(cp)^2}\approx mc^2+p^2/2m$, by $\alpha_x\alpha_y=i\sigma_z$ we have:
...
The term appeared in relativistic deriviation of electron schrodinger equation with electrical field $U(r)$ is often doubed as spin-orbital coupling (SOC). However, we should notice to arrive to this term, we introduce first order expansion of $\vert\psi_2\rangle$. Indeed, there are several method to arrive to this, each can bring different format of the relativistic corrections. We refer to [1] for a example.
