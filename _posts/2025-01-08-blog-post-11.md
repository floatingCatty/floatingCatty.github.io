...
For spin, basis is composed by $\vert s,m_s\rangle$, where $s=\frac{1}{2}$ for all electrons, while, $m_s$ can only taking value from $(-s,+s)$.
...
Since the $U_i(\vert r-r_i\vert)$ is the fully-relativistic pseudopotential, which is spherical, so $\xi_i$ will also be spherical. So, we can parameterize:
