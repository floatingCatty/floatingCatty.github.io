...
- We use $\Lambda_n$ as the space that $\mathbf{x}$ are defined on. So the volume should goes exponetial with $n$ as $\vert\Lambda_n\vert=C^n$ Where $C$ is some constant.
...
$$\frac{\partial p(x,t)}{\partial t}=\int dx'G(x,x')p(x',t)$$
 ...
 given the transition probability $\pi(x\vert x')$, and $S=\frac{1}{t}\int_0^Tx(t)dt$, the SCGF can be wriiten as:
...
\end{align*}
$$
Where $\tilde{G}(k)=G+k\delta(x,x')$, $v[\tilde{G}](k)$ is the largest eigenvalue of matrix $\tilde{G}(k)$ at each k. We can derive the rate function of $S$ using $\lambda(k)$
...
$$
\begin{align*}
\rho(\mathbf{x})&=\operatorname*{argmax}_\psi\left\{\frac{\langle\psi|\tilde{G}|\psi\rangle}{\langle\psi|\psi\rangle}\right\}\\
\end{align*}
$$
